...
const VERIFIER_PATH: &str = "scripts/verify-m050-s01.sh";
...
fn artifact_dir(test_name: &str) -> PathBuf {
    route_free::artifact_dir("m050-s01", test_name)
}
...
fn load_verifier_source(artifacts: &Path) -> String {
    ...
    let verifier_source = route_free::read_and_archive(
        ...
        &contract_artifacts.join("verify-m050-s01.sh"),
    );
    let _ = route_free::read_and_archive(
        &repo_root().join("scripts/tests/verify-m050-s01-onboarding-graph.test.mjs"),
        &contract_artifacts.join("verify-m050-s01-onboarding-graph.test.mjs"),
    );
    let _ = route_free::read_and_archive(
    ...
    let _ = route_free::read_and_archive(
        &repo_root().join("scripts/verify-m049-s05.sh"),
        &contract_artifacts.join("verify-m049-s05.sh"),
    );
    verifier_source
}
...
fn validate_verifier_contract(source: &str) -> Vec<String> {
    ...
    require_includes(
        ...
        &[
            ...
            "Getting Started footer drifted",
            "Clustered Example footer drifted",
            "Clustered Example footer regressed to a self-link",
            "expected no proof-page footer links",
        ],
    );
    ...
    require_order(
        ...
        source,
        &[
            "run_expect_success m050-s01-onboarding-graph",
...
#[test]
fn m050_s01_verifier_replays_the_fast_env_free_docs_graph_stack() {
...
#[test]
fn m050_s01_contract_fails_closed_when_phase_order_drifts() {
    ...
    let mutated = verifier_source
        .replacen(
            GRAPH_TEST_COMMAND,
            "node --test scripts/tests/verify-m050-s01-onboarding-graph.test.mjs # moved-later",
            1,
        )
        .replacen(PRODUCTION_PROOF_COMMAND, GRAPH_TEST_COMMAND, 1)
        .replacen(
            "node --test scripts/tests/verify-m050-s01-onboarding-graph.test.mjs # moved-later",
            PRODUCTION_PROOF_COMMAND,
            1,
...
#[test]
fn m050_s01_contract_fails_closed_when_built_html_bundle_markers_disappear() {
