...
The current gap is not feature count. The current gap is trust. M028/S01-S04 now close the first four concrete proof gaps: the repo contains a working `reference-backend/` package that builds, starts, migrates, serves HTTP, persists jobs in Postgres, and lets a timer-driven worker move a durable row from `pending` to `processed`; the compiler-facing Rust harness proves migration truth, HTTP/DB/health job-state agreement, and two-instance exact-once processing without benign claim contention inflating worker failure signals; the same backend now has trustworthy formatter, test-runner, LSP, and doc/editor proof surfaces instead of toy-only tooling claims; and the package now ships an artifact-first boring deployment path with a staged native binary, checked-in deploy SQL, thin runtime scripts, staged-bundle smoke proof outside the repo root, and operator-facing docs for build-host versus runtime-host responsibilities. S07 now closes the remaining recovery proof gap on the backend surface itself: worker crash recovery crosses a real restart boundary owned by a watchdog/restarted-worker contract, abandoned `processing` rows are reclaimed through a stale-cutoff path, `/health` exposes degraded/recovering restart metadata coherently, and the canonical Rust harness proves worker-crash, restart-visibility, whole-process restart, migration, and deploy-smoke behavior together. The remaining milestone gap is therefore S08: reconcile the public README/docs/UAT surfaces so they point only at these now-green recovery-aware proof paths.
